...
(** * Thread Event *)
(** The ThreadEvent corresponds to (ThrdEvt) in paper. *)
(** In Coq formalization, the promise/reservation and cancel steps in paper are all called promise step. *)
Module ThreadEvent.
  Inductive t :=
  ...
  Definition is_nprm (e: t) : Prop := 
    match e with 
    | promise _ _ _ _ Memory.op_kind_add => False
    | _ => True 
    end.
  ...
  Definition is_at_or_out_step (e: t): Prop :=
    match e with
    | promise _ _ _ _ _ | silent | read _ _ _ _ Ordering.plain | write _ _ _ _ _ Ordering.plain => False
    | _ => True
    end.

  Definition is_na_step (e: t): Prop :=
    match e with
    | silent | read _ _ _ _ Ordering.plain | write _ _ _ _ _ Ordering.plain => True
    | _ => False
    end.
  
  Definition is_na_access (e: t): Prop := 
    match e with
    | read _ _ _ _ Ordering.plain | write _ _ _ _ _ Ordering.plain => True
    | _ => False
    end.
    
  Definition is_na_write (e: t): Prop := 
    match e with
    | write _ _ _ _ _ Ordering.plain => True
    | _ => False
    end.
  
    Definition is_na_read (e: t): Prop := 
      match e with 
      | read _ _ _ _ Ordering.plain => True
      | _ => False
      end.
...
Module Local.
  ...
  Lemma wf_init: Local.wf Local.init Memory.init.
  Proof.
    unfold init; eapply wf_intro; simpl; eauto.
    eapply TView.bot_wf.
  ...
  Inductive read_step (lc1:t) (mem1:Memory.t) (loc:Loc.t) (to:Time.t) (val:Const.t) (released:option View.t) (ord:Ordering.t) (lc2:t) (lo: Ordering.LocOrdMap): Prop :=
  | read_step_intro
      from
      tview2
      (GET: Memory.get loc to mem1 = Some (from, Message.concrete val released))
      (LO: Ordering.mem_ord_match ord (lo loc))
      ...
      read_step lc1 mem1 loc to val released ord lc2 lo
  .
  ...
  Inductive write_step (lc1:t) (sc1:TimeMap.t) (mem1:Memory.t)
                       ...
                       (lc2:t) (sc2:TimeMap.t) (mem2:Memory.t) (kind:Memory.op_kind) (lo: Ordering.LocOrdMap): Prop :=
  | write_step_intro
      promises2
      (LO: Ordering.mem_ord_match ord (lo loc))
      ...
      (SC2: sc2 = sc1):
      write_step lc1 sc1 mem1 loc from to val releasedm released ord lc2 sc2 mem2 kind lo
  .
  ...
  Inductive program_step:
    forall (e:ThreadEvent.t) (lc1:t) (sc1:TimeMap.t) (mem1:Memory.t) (lc2:t) (sc2:TimeMap.t) (mem2:Memory.t) (lo: Ordering.LocOrdMap), Prop :=
  | step_silent
      lc1 sc1 mem1 lo:
      program_step ThreadEvent.silent lc1 sc1 mem1 lc1 sc1 mem1 lo
  | step_read
      lc1 sc1 mem1 lo
      loc ts val released ord lc2
      (LOCAL: Local.read_step lc1 mem1 loc ts val released ord lc2 lo):
      program_step (ThreadEvent.read loc ts val released ord) lc1 sc1 mem1 lc2 sc1 mem1 lo
  | step_write
      lc1 sc1 mem1 lo
      loc from to val released ord lc2 sc2 mem2 kind
      (LOCAL: Local.write_step lc1 sc1 mem1 loc from to val None released ord lc2 sc2 mem2 kind lo):
      program_step (ThreadEvent.write loc from to val released ord) lc1 sc1 mem1 lc2 sc2 mem2 lo
  | step_update
      lc1 sc1 mem1 lo 
      loc ordr ordw
      tsr valr releasedr releasedw lc2
      tsw valw lc3 sc3 mem3 kind
      (AT: lo loc = Ordering.atomic)
      (LOCAL1: Local.read_step lc1 mem1 loc tsr valr releasedr ordr lc2 lo)
      (LOCAL2: Local.write_step lc2 sc1 mem1 loc tsr tsw valw releasedr releasedw ordw lc3 sc3 mem3 kind lo):
      program_step (ThreadEvent.update loc tsr tsw valr valw releasedr releasedw ordr ordw)
                   lc1 sc1 mem1 lc3 sc3 mem3 lo
  | step_fence
      lc1 sc1 mem1 lo
      ordr ordw lc2 sc2
      (LOCAL: Local.fence_step lc1 sc1 ordr ordw lc2 sc2):
      program_step (ThreadEvent.fence ordr ordw) lc1 sc1 mem1 lc2 sc2 mem1 lo
  | step_syscall
      lc1 sc1 mem1 lo
      e lc2 sc2
      (LOCAL: Local.fence_step lc1 sc1 Ordering.seqcst Ordering.seqcst lc2 sc2):
      program_step (ThreadEvent.syscall e) lc1 sc1 mem1 lc2 sc2 mem1 lo
  .
  ...
  Lemma read_step_future
        lc1 mem1 loc ts val released ord lc2 lo
        (STEP: read_step lc1 mem1 loc ts val released ord lc2 lo)
        (WF1: wf lc1 mem1)
        (CLOSED1: Memory.closed mem1):
    <<WF2: wf lc2 mem1>> /\
    <<TVIEW_FUTURE: TView.le (tview lc1) (tview lc2)>> /\
  ...
  Lemma write_step_future
        ...
        (STEP: write_step lc1 sc1 mem1 loc from to val releasedm released ord lc2 sc2 mem2 kind lo) 
  ...
  Lemma program_step_future
        e lc1 sc1 mem1 lc2 sc2 mem2 lo
        (STEP: program_step e lc1 sc1 mem1 lc2 sc2 mem2 lo)
        (WF1: wf lc1 mem1)
        (SC1: Memory.closed_timemap sc1 mem1)
        (CLOSED1: Memory.closed mem1):
    <<WF2: wf lc2 mem2>> /\
  ...
  Lemma read_step_disjoint
        lc1 mem1 lc2 loc ts val released ord lc lo
        (STEP: read_step lc1 mem1 loc ts val released ord lc2 lo)
        (WF1: wf lc1 mem1)
        (DISJOINT1: disjoint lc1 lc)
        (WF: wf lc mem1):
  ...
  Lemma write_step_disjoint
        lc1 sc1 mem1 lc2 sc2 loc from to val releasedm released ord mem2 kind lc lo
        (STEP: write_step lc1 sc1 mem1 loc from to val releasedm released ord lc2 sc2 mem2 kind lo)
        (WF1: wf lc1 mem1)
        (SC1: Memory.closed_timemap sc1 mem1)
        (CLOSED1: Memory.closed mem1)
  ...
  Lemma read_step_promises
        lc1 mem loc to val released ord lc2 lo
        (READ: read_step lc1 mem loc to val released ord lc2 lo):
  ...
  Lemma program_step_disjoint
        e lc1 sc1 mem1 lc2 sc2 mem2 lc lo
        (STEP: program_step e lc1 sc1 mem1 lc2 sc2 mem2 lo)
        (WF1: wf lc1 mem1)
        (SC1: Memory.closed_timemap sc1 mem1)
        (CLOSED1: Memory.closed mem1)
  ...
  Lemma update_step_ord_not_pln
    e lc1 sc1 mem1 lc2 sc2 mem2 lo
    loc tsr tsw valr valw releasedr releasedw ordr ordw
    (STEP: program_step e lc1 sc1 mem1 lc2 sc2 mem2 lo)
    (UPDATE: e = ThreadEvent.update loc tsr tsw valr valw releasedr releasedw ordr ordw):
    ordr <> Ordering.plain /\ ordw <> Ordering.plain.
  Proof.
    rewrite UPDATE in STEP.
    inv STEP.
    inv LOCAL1.
    inv LOCAL2.
    unfold Ordering.mem_ord_match in *.
    rewrite AT in *.
    split.
    destruct LO; destruct LO0; subst; unfold "<>"; intro; try discriminate.
    destruct H; subst; try discriminate.
    destruct H; subst; try discriminate.
    destruct LO; destruct LO0; subst; unfold "<>"; intro; try discriminate.
    destruct H0; subst; try discriminate.
    destruct H0; subst; try discriminate.
  Qed.
End Local.
