...
(** We define [wf_I] (well-formed invariant), since we find that, 
    in defining our thread-local simulation, every point that the invariant I for shared state 
    is required to hold the message mapping should be well-formed. 

    Thus, we require our invariant I to be well-formed to indicate 
    that, at any point, if the invariant I holds, the message mapping is well-formed. 
    It can simplifiy our thread-local simulation definition. 
         ...
         (forall (NA_STEP: ThreadEvent.is_na_step te),
             exists e_src' dset1 dset2 dset2',
               ...
               (** The definition [na_step_dset] encapsulates
                   ...
                   - restriction for memory reads,
                     if such memory read reads a location recorded in the delayed write set  *)
