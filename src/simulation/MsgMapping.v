...
(** [TMapInj]: message mapping related timemap. *)

(** [opt_ViewInj]: message mapping related message view. *)

(** [weak_MsgInj] requires that every concrete message in the target level
...
(** Message mapping maps a message in the location x with "to"-timestamp t1 in the target level 
    ...
    It corresponds to [TMap] defined in Figure 12 in our paper *)
Definition Mapping := Loc.t -> Time.t -> option Time.t.

(** ** Monotonic Message Mapping *)
...
Lemma View_opt_wf_inj_prsv
      inj R R'
      (VIEW_OP_WF: View.opt_wf R)
      (OPT_VIEW_INJ: opt_ViewInj inj R R')
      (CLOSED_VIEW: closed_opt_viewinj inj R)
      (MON_INJ: monotonic_inj inj):
  View.opt_wf R'.
Proof.
  inv VIEW_OP_WF. ss. destruct R'; ss.
  unfold ViewInj in OPT_VIEW_INJ. destruct view, t; ss. des.
  econs; eauto.
  unfold TMapInj in *; ss.
  unfold closed_viewInj in CLOSED_VIEW; ss. des.
  unfold closed_TMapInj in *.
  inv WF. econs. unfold TimeMap.le in *. ii.
  specialize (PLN_RLX loc). ss.
  specialize (CLOSED_VIEW loc). des.
  specialize (CLOSED_VIEW0 loc). des.
  exploit OPT_VIEW_INJ; eauto. ii; subst.
  exploit OPT_VIEW_INJ0; eauto. ii; subst.
  eapply monotonic_inj_implies_le_prsv; eauto.
  ss. destruct R'; ss.
Qed.

Lemma Message_wf_inj
      inj R R' loc to to'
      (MSG_WF: Time.le (View.rlx (View.unwrap R) loc) to)
      (OPT_VIEWINJ: opt_ViewInj inj R R')
      (CLOSED_OPT_VIEWINJ: closed_opt_viewinj inj R)
      (INJ_TO: inj loc to = Some to')
      (MON_INJ: monotonic_inj inj):
  Time.le (View.rlx (View.unwrap R') loc) to'.
Proof.
  destruct R; ss. destruct R'; ss.
  unfold closed_viewInj in CLOSED_OPT_VIEWINJ. des.
  unfold closed_TMapInj in *; ss.
  specialize (CLOSED_OPT_VIEWINJ loc). des.
  specialize (CLOSED_OPT_VIEWINJ0 loc). des.
  unfold ViewInj in *; ss. destruct t, t0; ss. des.
  unfold TMapInj in *; ss.
  exploit OPT_VIEWINJ; eauto. ii; subst.
  exploit OPT_VIEWINJ0; eauto. ii; subst.
  eapply monotonic_inj_implies_le_prsv; eauto.
  destruct R'; ss.
  unfold TimeMap.bot. eapply Time.bot_spec; eauto.
Qed.

Lemma closed_view_inj_prsv
      R R' mem mem' inj
      (MEM_CLOSED_VIEW: Memory.closed_view R mem)
      (VIEW_INJ: ViewInj inj R R')
      (SOUND: forall loc to from val R,
          Memory.get loc to mem = Some (from, Message.concrete val R) ->
          exists to' from' val' R', inj loc to = Some to' /\
                               Memory.get loc to' mem' = Some (from', Message.concrete val' R')):
  Memory.closed_view R' mem'.
Proof.
  inv MEM_CLOSED_VIEW. unfold ViewInj in *. destruct R, R'; ss. des.
  econs; eauto; ss.
  unfold Memory.closed_timemap in PLN.
  unfold Memory.closed_timemap. ii.
  specialize (PLN loc). des.
  exploit SOUND; eauto. ii; des.
  unfold TMapInj in VIEW_INJ. exploit VIEW_INJ; eauto. ii; subst.
  exists from', val', R'. eauto.
  unfold Memory.closed_timemap in RLX.
  unfold Memory.closed_timemap. ii.
  specialize (RLX loc). des. 
  exploit SOUND; eauto. ii; des.
  unfold TMapInj in VIEW_INJ0. exploit VIEW_INJ0; eauto. ii; subst.
  exists from', val', R'. eauto.
Qed.
...
Proof.
  ...
  ii. eapply SOUND in H. des; eauto.
Qed.
...
Lemma closed_tviewInj_read_fence
      tview or inj
      (CLOSED_TVIEWINJ: closed_tviewInj inj tview):
  closed_tviewInj inj (TView.read_fence_tview tview or).
Proof.
  unfold TView.read_fence_tview, closed_tviewInj in *; ss.
  des. splits; eauto.
  des_if; eauto.
Qed.
...
Lemma TMapInj_sc_fence
      tview_tgt sc_tgt tview_src sc_src inj
      (CUR_RLX_INJ: TMapInj inj (View.rlx (TView.cur tview_tgt)) (View.rlx (TView.cur tview_src)))
      (SC_INJ: TMapInj inj sc_tgt sc_src)
      (CLOSED_TVIEW: closed_tviewInj inj tview_tgt)
      (CLOSED_SC: closed_TMapInj inj sc_tgt)
      (MON_INJ: monotonic_inj inj):
  TMapInj inj
          (TView.write_fence_sc tview_tgt sc_tgt Ordering.seqcst)
          (TView.write_fence_sc tview_src sc_src Ordering.seqcst).
Proof.
...
Lemma View_opt_wf_inj_general
      ...
      (REL_VIEWINJ: ViewInj inj (TView.rel tview loc) (TView.rel tview' loc))
...
Lemma View_opt_wf_inj
  ...
  View.opt_wf (TView.write_released tview' sc' loc to' released' ord).
Proof.
  eapply View_opt_wf_inj_general; eauto.
  unfold TViewInj in TVIEW_INJ. des; eauto.
  ii. unfold TViewInj in TVIEW_INJ. des; eauto.
Qed.
  ...
  des_if; ss.
  {
    des_if; ss.
    {
      ...
      destruct releasedr, releasedr'; ss.
      {
        ...
        exploit CUR_VIEWINJ; eauto. ii; des.
        unfold TMapInj in x0. rewrite CLOSED_TVIEW3.
        eapply x0 in CLOSED_TVIEW3. rewrite CLOSED_TVIEW3; eauto.
        unfold TimeMap.singleton in *.
      ...
      }
      {
        ...
        clear TLE_WRITE0.
        eapply monotonic_inj_implies_le_prsv; eauto.
        clear - CUR_VIEWINJ CLOSED_TVIEW.
        exploit CUR_VIEWINJ; eauto. ii; des.
        unfold ViewInj in x.
    ...
    }
    {
      ...
      destruct releasedr, releasedr'; ss.
      {
        ...
        unfold TMapInj in REL_VIEWINJ0. exploit REL_VIEWINJ0; eauto. ii; subst. eauto.
        unfold TimeMap.singleton in *.
        try rewrite Loc_add_eq in *. eapply Time.le_lteq; eauto.
      }
      {
        ...
        clear TLE_WRITE0.
        eapply monotonic_inj_implies_le_prsv; eauto.
        clear - REL_VIEWINJ CLOSED_TVIEW.
        destruct tview, tview'; ss. 
        unfold_closed_tviewInj CLOSED_TVIEW.
        specialize (CLOSED_TVIEW loc). des.
        unfold closed_TMapInj in CLOSED_TVIEW4. specialize (CLOSED_TVIEW4 loc). des.
...
Lemma TLE_write_prs
  ...
  Time.le (View.rlx (View.unwrap (TView.write_released tview' sc' loc to' releasedr' ord)) loc) to'.
Proof.
  eapply TLE_write_prs_general; eauto.
  clear - TVIEW_INJ. unfold TViewInj in TVIEW_INJ. des; eauto.
  ii. clear - TVIEW_INJ. unfold TViewInj in TVIEW_INJ. des; eauto.
Qed.
...
Lemma eq_inj_refl: 
  ...
  eq_inj inj inj.
Proof.
  intro. unfold eq_inj. intros. eauto.
Qed.
...
Lemma opt_ViewInj_write_released_inj_general
...
Proof.
  ...
  unfold TViewInj in *. des; eauto.
  ii. unfold TViewInj in *. des; eauto.
Qed. 
...
Proof.
  ...
  econs; eauto; ii.
  {
    ...
    rewrite x1 in GET; ss.
  }
  {
    exploit COMPLETE; eauto.
    ii; des. erewrite Memory.add_o; eauto.
    des_if; eauto. simpl in a. des1; subst.
...
Definition mem_injected (inj: Mapping) (mem: Memory.t) : Prop := 
  forall loc val f t R 
      (MSG: Memory.get loc t mem = Some (f, Message.concrete val R)),
      exists t',
      inj loc t = Some t'.

Lemma eq_inj_implies_mem_inj_dom_eq:
forall inj mem inj',
  mem_inj_dom_eq inj mem 
  -> 
  eq_inj inj inj'
...
Lemma closed_tm_to_closed_TMapInj
      inj mem mem' tm
      (CLOSED_TM: Memory.closed_timemap tm mem)
      (INJ_MEM: MsgInj inj mem mem'):
  closed_TMapInj inj tm.
Proof.
  unfold closed_TMapInj. ii.
  unfold Memory.closed_timemap in CLOSED_TM. specialize (CLOSED_TM loc).
  des.
  inv INJ_MEM.
  exploit SOUND; eauto. ii; des.
  eauto.
...
Lemma inj_init_eq_spec_inj_init_mem:
  inj_init = spec_inj Memory.init.
Proof.
  unfold spec_inj, inj_init.
  eapply functional_extensionality_dep. ii.
  eapply functional_extensionality_dep. ii.
  des_if; ss; subst; eauto.
  destruct (Memory.get x x0 Memory.init) eqn:GET_INIT; eauto.
  destruct p.
  unfold Memory.init, Memory.get in *; ss.
  rewrite Cell.init_get in GET_INIT. des_ifH GET_INIT; ss.
Qed.
...
Lemma MsgInj_init:
  MsgInj inj_init Memory.init Memory.init.
Proof.
  econs; eauto.
  - ii. unfold Memory.get, Memory.init in MSG; ss.
    rewrite Cell.init_get in MSG; ss. des_ifH MSG; ss. inv MSG; ss.
    exists Time.bot, Time.bot, (@None View.t).
    split; eauto.
  - ii. unfold inj_init in INJ; ss.
    des_ifH INJ; ss. inv INJ.
    unfold Memory.init, Memory.get; ss.
    rewrite Cell.init_get; eauto.
    des_if; ss; eauto.
