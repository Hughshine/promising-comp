...
Lemma msg_closed_mem_id_le
      mem mem' val R
      (MEM_LE: mem_id_le mem' mem)
      (MSG_CLOSED: Memory.closed_message (Message.concrete val R) mem):
  Memory.closed_message (Message.concrete val R) mem'.
Proof.
  inv MSG_CLOSED. inv MEM_LE; ss.
  inv CLOSED; ss.
  - inv CLOSED0; ss.
    econs; eauto. econs; eauto.
    unfold Memory.closed_timemap in PLN, RLX; ss.
    econs; eauto.
    {
      unfold Memory.closed_timemap. ii. exploit PLN; eauto.
      instantiate (1 := loc). i. des. eauto.
    }
    {
      unfold Memory.closed_timemap. ii. exploit RLX; eauto.
      instantiate (1 := loc). i. des. eauto.
    }
  - econs; eauto.
Qed.
...
Lemma mem_id_le_add_rsv_prsv
      mem1 loc from to mem1' mem2 mem2'
      (MEM_ID_LE: mem_id_le mem1 mem2)
...
Lemma Local_sc_fence_tview_le_prsv
      lc1 sc1 ordr ordw lc2 sc2 lc1' sc1'
      (FENCE: Local.fence_step lc1 sc1 ordr ordw lc2 sc2)
      (TVIEW_WF: TView.wf (Local.tview lc1))
      (TVIEW_LE: TView.le (Local.tview lc1') (Local.tview lc1))
      (MEM_ID_LE: mem_id_le (Local.promises lc1') (Local.promises lc1))
      (SC_LE: TimeMap.le sc1' sc1):
  exists lc2' sc2',
    <<FENCE_LE: Local.fence_step lc1' sc1' ordr ordw lc2' sc2'>> /\
    <<TVIEW_LE': TView.le (Local.tview lc2') (Local.tview lc2)>> /\
    <<SC_LE': TimeMap.le sc2' sc2>>.
Proof.
  destruct (Ordering.le Ordering.seqcst ordw) eqn:ORD; ss.
  {
    assert (ORD_CSEQ: ordw = Ordering.seqcst).
    {
      destruct ordw; ss.
    }
    subst.
    inv FENCE; ss. do 2 eexists.
    split.
    {
      econs; eauto.
      introv ORDER.
      eapply memory_nonsynch_mem_le; eauto.
      i. exploit PROMISES; eauto. ii.
      eapply mem_id_le_bot; eauto.
    }
    ss. split.
    {
  ...
  }
  {
    ...
    ii. subst. ss.
    instantiate (1 := sc1'). i. des.
    do 2 eexists. splits; eauto.
    clear - FENCE SC_LE. inv FENCE; ss.
    unfold TView.write_fence_sc, TView.read_fence_tview; ss.
    des_if; eauto. des_if; eauto.
    unfold TimeMap.le, TimeMap.join; ii.
...
Lemma Local_promise_tview_prsv
      lc1 mem1 loc from to msg lc2 mem2 kind lc1' mem1'
      (PROMISE: Local.promise_step lc1 mem1 loc from to msg lc2 mem2 kind)
      (MEM_LE: mem_id_le mem1' mem1)
      (PROM_LE: (Local.promises lc1') = (Local.promises lc1))
      (TVIEW_LE: TView.le (Local.tview lc1') (Local.tview lc1))
      (LOCAL_WF: Local.wf lc1 mem1)
      ...
      (MEM_CLOSED': Memory.closed mem1'):
  exists lc2' mem2',
    <<LE_PRM: Local.promise_step lc1' mem1' loc from to msg lc2' mem2' kind>> /\ 
    <<MEM_LE': mem_id_le mem2' mem2>> /\ 
    <<PROM_LE': (Local.promises lc2') = (Local.promises lc2)>> /\
    <<TVIEW_LE': TView.le (Local.tview lc2') (Local.tview lc2)>>.
Proof.
  inv PROMISE; ss. inv PROMISE0; ss.
  - (* add *)
    exploit MemoryProps.add_succeed_wf; eauto.
    ii; des.
    assert (MEM_ADD': exists mem2', Memory.add mem1' loc from to msg mem2').
    {
      eapply Memory.add_exists; eauto. inv MEM_LE; ss.
      ii. destruct msg2.
      eapply SOUND in GET2; eauto. do 2 des1.
      eapply DISJOINT in GET2; eauto.
      eapply SOUND_RSV in GET2; eauto.
      eapply DISJOINT in GET2; eauto.
    }
    des1.
    destruct lc1, lc1'; ss; subst.
    exists (Local.mk tview0 promises2) mem2'.
    splits; ss; eauto.
    {
      econs; eauto; ss. econs; eauto.
      ii. inv MEM_LE; ss.
      destruct msg'.
      eapply SOUND in GET; eauto. des. eauto.
      eapply SOUND_RSV in GET; eauto.
      destruct msg; eauto.
      exploit mem_id_le_add_prsv; [eapply MEM_LE | eauto..].
      eapply View.View.opt_le_PreOrder_obligation_1; eauto.
      i. eapply msg_closed_mem_id_le; eauto.
    }
    {
      destruct msg.
      eapply mem_id_le_add_prsv; [eapply MEM_LE | eapply MEM_ADD' | eapply MEM | eauto..].
      destruct released; ss. econs; eauto.
      eapply View.View.le_PreOrder_obligation_1; eauto.
      eapply mem_id_le_add_rsv_prsv; eauto.
    }
  - (* split *)
    des; subst; ss.
    exploit MemoryProps.split_succeed_wf; eauto.
    ii; des. 
    assert (MEM_SPLIT': exists mem2',
               Memory.split mem1' loc from to ts3
                            (Message.concrete val'0 released'0) (Message.concrete val' released') mem2').
    {
      eapply Memory.split_exists; eauto.
      exploit Memory.split_get0; [eapply PROMISES | eauto..]. i; des.
      rewrite <- PROM_LE in GET0. inv LOCAL_WF'; ss.
      eapply PROMISES0 in GET0; eauto.
    }
    des.
    destruct lc1, lc1'; ss; subst.
    exists (Local.mk tview0 promises2) mem2'.
    ...
    {
      econs; eauto. econs; eauto.
      exploit mem_id_le_split_prsv; [eapply MEM_LE | eauto..].
      eapply View.View.opt_le_PreOrder_obligation_1; eauto.
      i. eapply msg_closed_mem_id_le; eauto.
    }
    {
      eapply mem_id_le_split_prsv; [eapply MEM_LE | eapply MEM_SPLIT' | eapply MEM | eauto..].
      eapply View.View.opt_le_PreOrder_obligation_1; eauto.
    }
  - (* lower *)
    des; subst.
    exploit MemoryProps.lower_succeed_wf; eauto.
    ii; des.  inv MSG_LE; ss.
    assert (MEM_LOWER': exists mem2',
               Memory.lower mem1' loc from to
                            (Message.concrete val released) (Message.concrete val released0) mem2').
    {
      eapply Memory.lower_exists; eauto.
      exploit Memory.lower_get0; [eapply PROMISES | eauto..]. i; des.
      rewrite <- PROM_LE in GET0. inv LOCAL_WF'; ss.
      eapply PROMISES0 in GET0; eauto.
    }
    des.
    destruct lc1, lc1'; ss; subst.
    exists (Local.mk tview0 promises2) mem2'. s.
    splits; eauto.
    {
      econs; eauto.
      exploit mem_id_le_lower_prsv; [eapply MEM_LE | eauto..].
      eapply View.View.opt_le_PreOrder_obligation_1; eauto.
      i. eapply msg_closed_mem_id_le; eauto.
    }
    {
  ...
  - (* cancel *)
    exploit cancel_tview_le_prsv; [| eapply MEM_LE | eapply PROM_LE | eauto..].
    econs; eauto.
    i. des; ss; subst.
    exists lc2' mem2'. splits; eauto.
    inv LE_WRITE; ss; eauto.
Qed.
